  ...
  implements OnInit, OnChanges, OnDestroy
{
  ...
  handleDependantFieldChanges() {
    merge(..._.map(this.depends, (depend) => depend.valueChanges))
      .pipe(
        tap(() => {
    ...
    merge(..._.map(this.depends, (depend) => depend.statusChanges))
      .pipe(
        tap(() => {
          this.checkIfDependsIsInvalid();
          this.isDependsEmpty = _.some(
            _.map(this.depends, (depend) => depend.value),
            _.isEmpty
          );
        }),
        takeUntil(this.dispose$)
      ...
      .pipe(
        tap((value) => {
          ...
          if (!_.isEmpty(this.formControlRef.value)) {
            ...
            this.formGroup.lastChangedField = {
              code: this.field.code,
              value: this.formControlRef.value,
              sourceCategory: this.field.sourceCategory,
            };
          }
  ...
  fetchDependencyTerms() {
    if (!_.isEmpty(this.dependencyTerms) && !_.isEmpty(this.depends)) {
      const filterDependencyTerms =
        this.filterDependencyTermsByLastChangedValue();
      ...
      if (!_.isEmpty(filteredTerm)) {
        const tempAssociation = _.filter(
          _.compact(_.flatten(_.map(filteredTerm, "associations"))),
          (association) => {
            return this.field.sourceCategory
              ? _.toLower(association.category) ===
                  _.toLower(this.field.sourceCategory)
  ...
  getParentValue() {
    return (
      (!_.isEmpty(this.latestParentValue) && this.latestParentValue) ||
  ...
  filterDependencyTermsByLastChangedValue() {
    ...
    if (
      ...
      _.includes(this.field.depends, field.code)
    ) {
  ...
  checkIfDependsIsInvalid() {
    this.isDependsInvalid = _.includes(
      _.map(this.depends, (depend) => depend.invalid),
      true
    );
  ...
  onSubmit() {
    ...
    this.formControlRef.patchValue(
      this.isMultiple ? finalValue : finalValue[0]
    );
  ...
  openModal(event) {
    ...
    const isModalAlreadyOpened = modalElements.some(
      (element: HTMLElement) => element.hidden === false
    );
  ...
  private setupOptions() {
    ...
    if (this.isOptionsArray()) {
      ...
      from(
        (this.options as DynamicFieldConfigOptionsBuilder<any>)(
          this.formControlRef,
          this.depends,
          this.formGroup,
        ...
        .pipe(
          tap((options = []) => {
            ...
            this.resolvedOptions.forEach((option) => {
              const value: any = !_.isEmpty(this.field.output)
                ? option.get(this.field.output)
                : option.get("name") ||
                  option.get("identifier") ||
                  option.get("value") ||
                  option;
              const labelVal: any =
                option.get("name") || option.get("label") || option;

              this.optionValueToOptionLabelMap =
                this.optionValueToOptionLabelMap.set(value, labelVal);
            });
    ...
    if (this.resolvedOptions) {
      this.resolvedOptions.forEach((option) => {
        ...
        this.optionValueToOptionLabelMap = this.optionValueToOptionLabelMap.set(
          value,
          labelVal
        );
      });
  ...
  isAllSelected() {
    if (this.isOptionsArray()) {
      if (
        this.default &&
        this.default.length > 1 &&
        this.default.length == this.options.length
      ) {
